...
#         return dp[0][0]
    ...
    # or---
    def tabulation_N_2K(self, n, k, prices):
        dp = [[0 for i in range(k*2+1)] for j in range(len(prices)+1)]
    ...
    def maxProfit(self, k: int, prices: List[int]) -> int:
        dp = [[[-1 for i in range(k+1)] for j in range(2+1)]
              for _ in range(len(prices)+1)]

        # return self.memo(0,prices,0,dp,k)\
        return self.tabulation_N_2K(len(prices), k, prices)
